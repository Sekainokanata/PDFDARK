...
window.processSvgImagesHighQuality = async function processSvgImagesHighQuality(svgRoot, options = {}) {
  ...
  for (const imgEl of images) {
    try {
      ...
      if (!href) continue;
      // オリジナル href を一度だけ保存（復元用）
      if (!imgEl.hasAttribute('data-dm-original-href')) {
        imgEl.setAttribute('data-dm-original-href', href);
      }
...
window.detectPhotoRegionsPythonStyle = function detectPhotoRegionsPythonStyle(imgDataSmall, origW, origH, options = {}){
  console.log('detectPhotoRegionsPythonStyle called', imgDataSmall, origW, origH, options);
  try {
    if (!imgDataSmall || !imgDataSmall.data) return { ok:false, boxes:[], reason:'no image data' };
...
window.convertPageToPng = async function convertPageToPng(page, viewport, paper){
  ...
  // ===== ダークモード OFF の場合はオリジナルをそのまま配置して終了 =====
  if (!window.__viewer_darkModeEnabled) {
    try {
      // 復元用にオリジナル ImageData を保存（ON に切り替わった時再処理できる）
      canvas.__originalImageData = imgData;
      canvas.__darkProcessed = false;
    } catch(_) {}
    paper.appendChild(canvas);
    return canvas;
  }
    ...
    if (boxesPx.length === 0) {
      ...
      try {
        ...
        if (isPhotoLike) {
          ...
          canvas.__originalImageData = imgData; // for consistency
          canvas.__darkProcessed = true;
          paper.appendChild(canvas);
          return canvas;
        }
      } catch(_) {}
